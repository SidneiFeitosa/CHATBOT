# CHATBOT
# Assistente Virtual
Chatbot baseado em RAG (Retrieval Augmented Generation) usando a API GPT3.5 ou similar abaixo segue o relatório com informações sobre Implementação (bibliotecas e funções utilizadas para o projeto), Avaliação (testes manuais para analisar a precisão do projeto), Instalação e execução (mini guia para execução do projeto localmente).  
